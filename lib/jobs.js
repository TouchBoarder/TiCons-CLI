...
function _meetsSpec(name, cfg) {
  ...
  if (spec.type === 'splash') {
	...
	if (cfg.nineDpi > 0 && spec.dpi !== cfg.nineDpi && _.indexOf(spec.platforms, 'android') !== -1){
		return false;
	}
  }
...
function _initSpec(name, cfg) {
  ...
  if (cfg.nine && spec.ninePath) {
	...
	if (cfg.nineDpi > 0){
		spec.ninePath = path.join(':assets:', 'Android', 'Resources', 'drawable', 'background.9.png');
	}
  ...
  var iOSOnly = _.difference(cfg.platforms, ['ios', 'iphone', 'ipad']).length === 0;
  var isTargetedPlatform = (cfg.platforms.length === 1);
  
  if(iOSOnly || isTargetedPlatform){
	  _path = _path.replace('Resources', '');
	  _path = _path.replace('Android', '');
	  _path = _path.replace('iOS', '');
	  _path = _path.replace('windows', '');
	  // _path = _path.replace('mobileweb', '');
	  // _path = _path.replace('tizen', '');
	  // _path = _path.replace('blackberry', '');
  }
